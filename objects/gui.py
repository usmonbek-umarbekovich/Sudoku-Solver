...
class Sudoku:
  def __init__(self):
    ...
    self.run_button = tk.Button(frame, text="Solve", **style.run_button)
  ...
  def write_given_numbers(self, width, height, step_x, step_y):
    ...
    while y < height:
      ...
      x = step_x/2
      while x < width:
        self.canvas.create_text(x, y, text=str(count), anchor=tk.CENTER, font="Arial 16")
        sleep(0.05)
        self.canvas.update()
        x += step_x
        count += 1
